...
State *is* the badge's identity, but the literal word carries it: a verified domain renders a **solid green pill** with a bold "Verified" and a white check; Building is **solid amber** with a bold "Building" and a white dot. Color + icon alone are ambiguous to strangers (an amber pill with a dot can read as "also approved" to someone with no Witnessed mental model); the state word removes the guesswork. No progress ring, no 0–100 readout — the color + word do all the heavy lifting, and the precise 0–100 number lives on the seal page where there's room for the detail.

Canvas is a **Split Pill** (current ETag: v16): state-tinted LEFT half (icon + state word, fixed 104px) + theme-aware neutral RIGHT half carrying **the actual domain** (`acme.com`, monospace, adaptive width up to a 26-char tail-truncation cap). Earlier iterations (v13–v15) hard-coded `witnessed.cc` as the right-half wordmark; that read as "witnessed.cc is verified" on every page and was the wrong call. Brand attribution now lives in the click target (`witnessed.cc/b/<domain>`), not in pixels. Theme variance is reintroduced via `?theme=light|dark` (default `dark` for back-compat with pre-v16 URLs) so owners on white-bg email clients (Gmail web) and dark-bg clients (Apple Mail dark) both get a right half that doesn't fight their chrome — the seal-page preview mirrors the site theme into the copied HTML, so what they see is what they paste. Dimension constants live in `lib/badge-dimensions.ts` and are shared by the route, `BadgeEmbed`, and the landing-page demo so the rendered image and the `<img>` tag's advertised size stay in lockstep. State resolution lives in `lib/badge-state.ts#resolveSnapshot` so tests can import it without loading `next/og`. Cached at the edge with an `ETag` keyed on `(state, theme, format, layout-version)` — domain isn't part of the ETag because each badge URL already keys by domain. The `?preview=verified|building` query short-circuits the DB lookup for marketing surfaces; it never mutates data.
...
**Badge.** See `Dynamic badge (implemented)` above for the full rendering contract. Short surface: `GET /badge/[slug]` renders SVG or PNG (format from the slug suffix); `?preview=verified|building` short-circuits the DB lookup for marketing surfaces; `?theme=light|dark` flips the right-half palette (default `dark`). Bump the trailing layout fingerprint in `cacheHeaders()` (currently `v16`) whenever the visual output changes so in-the-wild 304s don't serve stale pixels.
