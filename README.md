...
- **Dynamic badge** at `/badge/[domain]` — SVG or PNG, state-colored pill (`[ icon ][ STATE WORD ] · [ domain ]`): solid green "Verified", solid amber "Building", outline gray "Pending". State word included inline so strangers don't have to decode color alone. `ETag`-cached by state; `?preview=verified|onRecord|pending` for marketing surfaces
